...
static inline int ff_clampi(int x, int low, int high) { return ff_maxi(low, ff_mini(x, high)); }

static inline float ff_clampf(float x, float low, float high) {
    return ff_maxf(low, ff_minf(x, high));
}

static inline int ff_absi(int x) { return x < 0 ? -x : x; }

static inline float ff_absf(float x) { return x < 0 ? -x : x; }
